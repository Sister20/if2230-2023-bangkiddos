 ...
 * @param write_bit             If the bit set, read/write. Otherwise, read-only
 * @param user_supervisor       Control access to the page based on privilege level
 * @param write_through         If the bit set, write-through caching's enabled. Otherwise, write_back's enabled
 * @param cache_disable         If the bit set, page won't be cached. Otherwise, caching's enabled
 * @param accessed              Indicate whether PDE war read during virtual address translation
 * @param dirty                 Indicate whether a page has been written to
 * @param use_pagesize_4_mb     Stores the page size for specific entry
 */
 ...
 * @param available             Ignored
 * @param page_attribute_table  Indicate the memory caching type
 * @param higher_address        Physical address
 * @param reserved              Must be 0
 * @param lower_address         Physical address
 * Note:
